      ...
      {files.length === 0 ? (
        <div className="flex items-center justify-center gap-2.5 py-8 px-6">
          ...
          <p className={`text-sm transition-colors ${dragging ? 'text-indigo-300' : 'text-zinc-400'}`}>
            Drop files here or <span className="text-zinc-300">click to browse</span>
          </p>
          <span className="text-zinc-700 text-xs">· 500 MB max</span>
        </div>
      ) : (
        <div className="p-3 space-y-1.5">
          {files.map((f, i) => (
            <div key={i} className="flex items-center gap-3 bg-zinc-800 rounded-lg px-3 py-2.5">
              ...
              <span className="text-sm text-zinc-200 truncate flex-1">{f.name}</span>
              <span className="text-xs text-zinc-500 shrink-0">{formatSize(f.size)}</span>
            </div>
          ))}
          <p className="text-xs text-zinc-600 text-center pt-1 pb-1">Click to add more files</p>
        </div>
      )}
    </div>
  )
